...
class Ui {
  static GetSnackBar SuccessSnackBar({String title = 'Success', required String message}) {
    ...
    return GetSnackBar(
      titleText: Text(title.tr, style: Get.textTheme.headlineMedium?.merge(const TextStyle(color: Colors.white))),
      messageText: Text(message, style: Get.textTheme.labelSmall?.merge(const TextStyle(color: Colors.white))),
  ...
  static GetSnackBar ErrorSnackBar({String title = 'Error', required String message}) {
    ...
    return GetSnackBar(
      ...
      messageText: Text(message.substring(0, min(message.length, 200))
          , style: Get.textTheme.labelSmall?.merge(const TextStyle(color:   Colors.white))),
      snackPosition: SnackPosition.BOTTOM,
      margin: const EdgeInsets.all(20),
